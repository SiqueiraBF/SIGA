...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // Set Vercel ISR Cache Control: Cache for 30 mins, serve stale up to 24h while revalidating
    ...
    try {
        ...
        // Fetch Nuntec Credentials Securely (Backend Only)
        ...
        if (!config || !config.is_active || !config.base_url || !config.username) {
            return res.status(500).json({ error: 'Nuntec Integration is disabled or incorrectly configured.' });
        }

        const authHeader = 'Basic ' + Buffer.from(`${config.username}:${config.password}`).toString('base64');
        const fetchCfg = { headers: { Authorization: authHeader } };
        ...
        // --- DT-15: Grouping logic for split deliveries (Truck split / Multiple truckloads) ---
        ...
        rawMeasurements.forEach(m => {
            ...
            if (!measurementsMap.has(key)) {
                measurementsMap.set(key, { 
                    ...
                    // Weighting density and temp by net weight for accuracy
            ...
            // Weighted average by weight (more accurate than simple average for split loads)
        ...
        // Helper to check if two dates are within X days
        ...
        validSupplies.forEach(supply => {
            ...
            const match = measurements.find(m => {
                ...
                // Refinamento DT-16: Compare Base NF instead of checking tokens to avoid collisions (e.g., match "1" to "1379-1")
                // Also add a date proximity check (+/- 10 days) to avoid matching same NF numbers from different months
        ...
        console.error('Audit API Serverless Error:', error);
        return res.status(500).json({ error: error.message || 'Internal Serverless Error fetching Nuntec Data.' });
    }
}
