...
import { X, Calendar, User, MapPin, Sparkles, Trash2 } from 'lucide-react';
import { CleaningRegistry } from '../../services/cleaningService';
import { useCleaningDelete } from '../../hooks/useCleaningDelete';
...
export function CleaningDetailsModal({
  isOpen,
  onClose,
  registry,
  onDeleteSuccess,
}: CleaningDetailsModalProps) {
  const { user, role } = useAuth();
  const isAdmin = role?.nome === 'Administrador' || (user as any)?.funcao === 'Administrador';
  const deleteMutation = useCleaningDelete();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-slate-700">
                      {format(new Date(registry.data + 'T12:00:00'), "dd 'de' MMMM 'de' yyyy", {
                        locale: ptBR,
                      })}
                    </p>
                  </div>
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">
              Evidências Fotográficas ({registry.fotos?.length || 0})
            </h3>
            {registry.fotos && registry.fotos.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {registry.fotos.map((foto, idx) => (
                  <div
                    key={idx}
                    className="group relative aspect-square bg-slate-100 rounded-lg overflow-hidden border border-slate-200 cursor-pointer shadow-sm hover:shadow-md transition-all"
                  >
                    <img
                      src={foto}
                      alt={`Evidência ${idx + 1}`}
