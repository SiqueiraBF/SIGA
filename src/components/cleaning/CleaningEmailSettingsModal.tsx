...
interface CleaningEmailSettingsModalProps {
  onClose: () => void;
}

const schema = z.object({
  email_limpeza_to: z.string().min(1, 'Obrigatório'),
  email_limpeza_cc: z.string().optional(),
});

type FormData = z.infer<typeof schema>;

export function CleaningEmailSettingsModal({ onClose }: CleaningEmailSettingsModalProps) {
  ...
  const loadSettings = async (selectedFarmId: string) => {
    ...
    const keyTo = selectedFarmId ? `email_limpeza_to_${selectedFarmId}` : 'email_limpeza_to';
    const keyCc = selectedFarmId ? `email_limpeza_cc_${selectedFarmId}` : 'email_limpeza_cc';

    const params = await systemService.getParameters([keyTo, keyCc]);

    setValue('email_limpeza_to', params[keyTo] || '');
    setValue('email_limpeza_cc', params[keyCc] || '');
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const keyTo = farmId ? `email_limpeza_to_${farmId}` : 'email_limpeza_to';
      const keyCc = farmId ? `email_limpeza_cc_${farmId}` : 'email_limpeza_cc';

      await Promise.all([
        systemService.updateParameter(keyTo, data.email_limpeza_to),
        systemService.updateParameter(keyCc, data.email_limpeza_cc || ''),
      ]);
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200"
      onClick={onClose}
    >
      <div
      ...
      >
        <ModalHeader
          title="Notificações de Limpeza"
          subtitle="Defina quem recebe os relatórios de limpeza"
          icon={Mail}
          onClose={onClose}
        />
            ...
            <form id="email-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">
                  ...
                  {farmId && <span className="text-blue-600 text-xs">(Específico)</span>}
                </label>
                <textarea
                  {...register('email_limpeza_to')}
                ...
                {errors.email_limpeza_to && (
                  <span className="text-red-500 text-xs">{errors.email_limpeza_to.message}</span>
                )}
              </div>
