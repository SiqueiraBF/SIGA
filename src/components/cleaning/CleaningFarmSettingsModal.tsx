...
export function CleaningFarmSettingsModal({ onClose }: CleaningFarmSettingsModalProps) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await systemService.updateParameter(
        'cleaning_monitored_farms',
        JSON.stringify(selectedFarmIds),
      );
              ...
              <div className="divide-y divide-slate-100">
                {farms.map((farm) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`font-medium ${isSelected ? 'text-blue-700' : 'text-slate-600'}`}
                      >
                        {farm.nome}
                      </span>
                      <div
                        className={`w-6 h-6 rounded-full border flex items-center justify-center transition-colors ${isSelected ? 'bg-blue-500 border-blue-500 text-white' : 'border-slate-300 bg-white'}`}
                      >
                        {isSelected && (
                          <svg
                            className="w-4 h-4"
