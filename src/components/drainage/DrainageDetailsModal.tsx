...
import { useDrainageDelete } from '../../hooks/useDrainageDelete';
...
export function DrainageDetailsModal({ isOpen, onClose, drainage }: DrainageDetailsModalProps) {
  const { user, role } = useAuth();
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [selectedImageUrl, setSelectedImageUrl] = useState<string | null>(null);
  const [deleting, setDeleting] = useState(false);
  const { mutateAsync: deleteDrainage } = useDrainageDelete();
  ...
  const handleDelete = async () => {
    if (confirm('Tem certeza que deseja excluir este registro de drenagem?')) {
      setDeleting(true);
      try {
        await deleteDrainage(drainage.id);
        toast.success('Registro de drenagem excluído!');
        onClose();
      } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div>
                ...
                <div className="text-2xl font-bold text-blue-600 flex items-baseline gap-1">
                  {drainage.litros_drenados.toLocaleString('pt-BR')}{' '}
                  <span className="text-sm">Litros</span>
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {drainage.fotos.map((url, idx) => (
                    <button
                      ...
                      className="group relative aspect-square bg-slate-100 rounded-lg overflow-hidden border border-slate-200 block hover:shadow-md transition-all outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <img
                        src={url}
                        alt={`Foto ${idx + 1}`}
