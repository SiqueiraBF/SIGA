...
export function DrainageFormModal({ isOpen, onClose, initialData }: DrainageFormModalProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || (!user.fazenda_id && !initialData)) {
      toast.error('Erro: Usuário sem fazenda vinculada.');
      return;
    }
    ...
    setSubmitting(true);
    try {
      await submitDrainage({
        drainage: {
          posto_id: postoId,
          fazenda_id: initialData?.fazenda_id || user.fazenda_id!,
          usuario_id: initialData?.usuario_id || user.id,
      ...
      toast.success(
        initialData ? 'Drenagem atualizada com sucesso!' : 'Drenagem registrada com sucesso!',
      );
      onClose();
    } catch (error: any) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <MultiSelect
                options={[
                  {
                    value: 'Límpido e Isento',
                    label: 'Límpido e Isento (Transparente, sem partículas)',
                  },
                  { value: 'Turvo', label: 'Turvo (Perda de transparência, possível umidade)' },
                  {
                    value: 'Com Água Livre',
                    label: 'Com Água Livre (Separação clara de água no fundo)',
                  },
            ...
            <div className="md:col-span-2">
              ...
              {tanquesAdicionais.length === 0 && (
                <p className="text-xs text-slate-400 mt-1">
                  Este posto não possui tanques adicionais cadastrados.
                </p>
              )}
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-xs text-slate-400">
                Permitido imagens JPG, PNG. Múltiplos arquivos.
              </p>
            </div>
          </div>
