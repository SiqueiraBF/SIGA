...
interface GoodsReceiptEmailSettingsModalProps {
    onClose: () => void;
}

const schema = z.object({
    email_cd_to: z.string().min(1, 'Obrigatório'),
    email_cd_cc: z.string().optional(),
});

type FormData = z.infer<typeof schema>;

export function GoodsReceiptEmailSettingsModal({ onClose }: GoodsReceiptEmailSettingsModalProps) {
    ...
    const loadSettings = async (selectedFarmId: string) => {
        setIsLoading(true);
        const keyTo = selectedFarmId ? `email_cd_to_${selectedFarmId}` : 'email_cd_to';
        const keyCc = selectedFarmId ? `email_cd_cc_${selectedFarmId}` : 'email_cd_cc';

        const legacyKeyTo = selectedFarmId ? `email_entrada_to_${selectedFarmId}` : 'email_entrada_to';
        const legacyKeyCc = selectedFarmId ? `email_entrada_cc_${selectedFarmId}` : 'email_entrada_cc';

        const params = await systemService.getParameters([keyTo, keyCc, legacyKeyTo, legacyKeyCc]);

        setValue('email_cd_to', params[keyTo] || params[legacyKeyTo] || '');
        setValue('email_cd_cc', params[keyCc] || params[legacyKeyCc] || '');
    ...
    const onSubmit = async (data: FormData) => {
        try {
            const keyTo = farmId ? `email_cd_to_${farmId}` : 'email_cd_to';
            const keyCc = farmId ? `email_cd_cc_${farmId}` : 'email_cd_cc';

            await Promise.all([
                systemService.updateParameter(keyTo, data.email_cd_to),
                systemService.updateParameter(keyCc, data.email_cd_cc || '')
            ]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200" onClick={onClose}>
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
                <ModalHeader
                    title="Notificações da Central (CD)"
                    subtitle="Defina quem recebe os avisos de Entrada e Saída (Expedição) da Fazenda"
                    icon={Mail}
                    onClose={onClose}
                />
                ...
                <div className="p-6 space-y-4">
                    ...
                    {isLoading ? (
                        ...
                        <form id="email-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            <div className="pt-2">
                                <h4 className="font-bold text-slate-800 border-b pb-2 mb-3">Notificações CD (Entrada e Expedição)</h4>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        {errors.email_cd_to && <span className="text-red-500 text-xs">{errors.email_cd_to.message}</span>}
                                    </div>
