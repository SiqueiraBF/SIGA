...
interface StockEmailSettingsModalProps {
    onClose: () => void;
}

const schema = z.object({
    email_estoque_to: z.string().min(1, 'Obrigatório'),
    email_estoque_cc: z.string().optional(),
});

type FormData = z.infer<typeof schema>;

export function StockEmailSettingsModal({ onClose }: StockEmailSettingsModalProps) {
    ...
    const loadSettings = async () => {
        setIsLoading(true);
        const params = await systemService.getParameters(['email_estoque_to', 'email_estoque_cc']);
        setValue('email_estoque_to', params['email_estoque_to'] || '');
        setValue('email_estoque_cc', params['email_estoque_cc'] || '');
        setIsLoading(false);
    };
    ...
    const onSubmit = async (data: FormData) => {
        try {
            await Promise.all([
                systemService.updateParameter('email_estoque_to', data.email_estoque_to),
                systemService.updateParameter('email_estoque_cc', data.email_estoque_cc || '')
            ]);
                    ...
                    {isLoading ? (
                        ...
                        <form id="email-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            <div>
                                ...
                                {errors.email_estoque_to && <span className="text-red-500 text-xs">{errors.email_estoque_to.message}</span>}
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">Cópia (CC)</label>
                                <textarea
                                    {...register('email_estoque_cc')}
                                    className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"
