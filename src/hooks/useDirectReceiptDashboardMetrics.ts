...
export function useDirectReceiptDashboardMetrics(
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const receipts = await directReceiptService.getDirectReceipts({
                dataInicio,
                dataFim,
            ...
            const totalValue = receipts.reduce((sum, r) => sum + Number(r.valor || 0), 0);
            ...
            const receiptsComAtraso = receipts.map(r => {
            ...
            }).sort((a, b) => b.delayDays - a.delayDays); // Decrescente (maior atraso primeiro)
