...
export function CleaningList() {
    ...
    const [allFarmsStatus, setAllFarmsStatus] = useState<{ id: string; nome: string; almoxarifado: boolean; posto: boolean }[]>([]);
    ...
    const loadData = async () => {
        ...
        try {
            // Load History
            const data = await cleaningService.getCleanings(isAdmin ? {} : { fazenda_id: user.fazenda_id });
            setRegistries(data);
    ...
    const AdminStatusTable = () => (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-4 border-b border-slate-100 bg-slate-50">
                <h3 className="font-bold text-slate-700">Status Semanal por Fazenda</h3>
            </div>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-medium text-center">Almoxarifado (Seg)</th>
                            <th className="px-6 py-3 font-medium text-center">Posto (Sex)</th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <td className="px-6 py-4 text-center">
                                    {farm.almoxarifado ? (
                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            <CheckCircle size={12} className="mr-1" /> Feito
                                        </span>
                                    ) : (
