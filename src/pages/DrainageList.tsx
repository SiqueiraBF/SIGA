...
import { StationDrainage } from '../services/drainageService';
...
import { useDrainageData, useDrainageStationsData } from '../hooks/useDrainageData';
...
export function DrainageList() {
  ...
  // Dependent Filters (Postos)
  const { stations: postos } = useDrainageStationsData(fazendaId);

  // Client-side Search
  const viewDrainages = useMemo(() => {
    if (!drainages) return [];
    if (searchText.trim() === '') return drainages;

    const lowerSearch = searchText.toLowerCase();
    return drainages.filter(d =>
      (d.posto?.nome || '').toLowerCase().includes(lowerSearch) ||
      (d.usuario?.nome || '').toLowerCase().includes(lowerSearch) ||
      (d.fazenda?.nome || '').toLowerCase().includes(lowerSearch)
    );
  }, [drainages, searchText]);
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {viewDrainages.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-slate-600">{item.usuario?.nome}</td>
                        <td className="px-6 py-4 text-right font-bold text-slate-700">
                          {item.litros_drenados.toLocaleString('pt-BR')} L
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-xs">
                            <span className="font-bold text-slate-600">Aspecto:</span>{' '}
                            {item.aspecto_residuo}
                          </div>
                          <div className="text-xs mt-1">
                            <span className="font-bold text-slate-600">Destino:</span>{' '}
                            {item.destino_residuo}
                          </div>
                        </td>
      ...
      {isSettingsOpen && <DrainageEmailSettingsModal onClose={() => setIsSettingsOpen(false)} />}
      ...
      <DrainageTutorialModal isOpen={isTutorialOpen} onClose={() => setIsTutorialOpen(false)} />
    </div>
  );
}
