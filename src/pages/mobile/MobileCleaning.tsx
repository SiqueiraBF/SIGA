...
export function MobileCleaning() {
  ...
  const handleSubmit = () => {
    ...
    const fazendaNome =
      fazendas.find((f) => f.id === fazendaId)?.nome ||
      user?.fazenda?.nome ||
    ...
    submitMutation.mutate(
      ...
      },
      {
        onSuccess: (data) => {
          if (!data.emailSent) {
            alert(
              'Registro salvo com sucesso, mas houve erro no envio do e-mail. A TI foi notificada.',
            );
          } else {
  ...
  return (
    <div className="min-h-full bg-slate-50 relative pb-28">
      ...
      <main className="p-4 space-y-6">
        ...
        <section>
          <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block mb-3 pl-1">
            Área Limpa *
          </label>
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <Box
                size={32}
                className={tipo === 'ALMOXARIFADO' ? 'text-teal-600' : 'text-slate-400'}
              />
        ...
        <section className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium">
              Capture o estado atual (Pelo menos 1 foto)
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {photoPreviews.map((src, idx) => (
              <div
                key={idx}
                className="relative aspect-square bg-slate-100 rounded-xl overflow-hidden border border-slate-200"
              >
        ...
        {/* Observações */}
        <section>
          <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block mb-2 pl-1">
