...
export const cleaningService = {
  ...
  async createCleaning(
    ...
    // 1. Upload das fotos primeiro para gerar as URLs (Bypassa necessidade de UPDATE no DB RLS)
    ...
    try {
      for (const file of photos) {
        ...
        const fileName = `${cleaning.fazenda_id}/${new Date().getFullYear()}/${cleaning.tipo}/pic_${Date.now()}_${uniqueId}.${fileExt}`;
      ...
      // Rollback: Remove as fotos que já subiram caso alguma falhe
      if (uploadedPaths.length > 0) {
        await supabase.storage.from('cleaning-photos').remove(uploadedPaths);
      }
      throw error;
    }

    // 2. Transação Única de INSERT com as fotos (evita erro PGRST116 de falta de UPDATE Policy)
    const { data: finalRecord, error: insertError } = await supabase
      .from('cleaning_registries')
      .insert({
        ...cleaning,
        fotos: photoUrls,
    ...
    if (insertError) {
      // Rollback: Apaga fotos do Storage se der erro de banco de dados
      if (uploadedPaths.length > 0) {
        await supabase.storage.from('cleaning-photos').remove(uploadedPaths);
      }
      throw insertError;
    ...
    return finalRecord as CleaningRegistry;
  },

  async deleteCleaning(id: string) {
    const { error } = await supabase.from('cleaning_registries').delete().eq('id', id);
  ...
  async getAllFarmsWeeklyStatus() {
    ...
    let query = supabase.from('fazendas').select('id, nome').eq('ativo', true).order('nome');
