...
export const nuntecService = {
  ...
  /**
    ...
    */
  async getStockMeasurements(allowedReservoirIds?: string[]): Promise<NuntecMeasurement[]> {
    const config = await this.getConfig();
   ...
   */
  async getStationsData(allowedReservoirIds?: string[]): Promise<NuntecReservoir[]> {
    const config = await this.getConfig();
    if (!config) return [];
    ...
    try {
      ...
      for (let i = 0; i < reservoirNodes.length; i++) {
        ...
        if (id && stationId) {
          if (allowedReservoirIds && allowedReservoirIds.length > 0 && !allowedReservoirIds.includes(id)) {
            continue;
          }
   ...
   */
  async getAdmeasurements(allowedReservoirIds?: string[]): Promise<NuntecAdmeasurement[]> {
    const config = await this.getConfig();
    if (!config) return [];
    ...
    try {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (id && nozzleId) {
          // We can't filter admeasurements by reservoir directly without station context here easily unless we have a nozzle->reservoir map.
          // However, StationManagement currently doesn't strictly need to filter admeasurements to save major egress as there aren't thousands per day.
          records.push({
            id: id,
   ...
   */
  /**
   * Fetches consumptions (fuelings) from Nuntec API for the last X days or specific range.
   * Uses /fuelings.xml endpoint.
   */
  async getConsumptions(daysOrStart: number | string = 7, endDate?: string, allowedReservoirIds?: string[]): Promise<NuntecConsumption[]> {
    const config = await this.getConfig();
    if (!config) return [];
    ...
    try {
      ...
      if (typeof daysOrStart === 'number') {
        const now = new Date();
        sinceDate = subHours(now, daysOrStart * 24);
        sinceStr = format(sinceDate, "yyyy-MM-dd'T'HH:mm:ss");
      } else if (daysOrStart) {
        sinceDate = parseISO(daysOrStart);
        sinceStr = `${daysOrStart}T00:00:00`;
      } else {
        // Default to 30 days if empty string passed
        const now = new Date();
        sinceDate = subHours(now, 30 * 24);
        sinceStr = format(sinceDate, "yyyy-MM-dd'T'HH:mm:ss");
      }

      // Determine End Date (for client-side filtering)
      ...
      if (resFuelings.ok) {
        ...
        if (!xmlText.includes('<html')) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            ...
            const endAtStr = getTagValue(node, 'end-at') || getTagValue(pointingNode, 'datetime');

            // Date Filter
            if (endAtStr) {
              const itemDate = parseISO(endAtStr);
            ...
            if (t.id && t['reservoir-id']) {
              if (allowedReservoirIds && allowedReservoirIds.length > 0 && !allowedReservoirIds.includes(t['reservoir-id'])) {
                continue;
              }
              consumptions.push(t);
            }
      ...
      if (resTransfers.ok) {
        ...
        if (!xmlText.includes('<html')) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            ...
            const dateStr = getTagValue(node, 'end-at') || getTagValue(node, 'end-date') || getTagValue(node, 'created-at');
            if (dateStr) {
            ...
            if (t.id && (t['reservoir-id'] || t['nozzle-id'])) {
              if (t['reservoir-id'] && allowedReservoirIds && allowedReservoirIds.length > 0 && !allowedReservoirIds.includes(t['reservoir-id'])) {
                continue;
              }
              consumptions.push(t);
            }
  ...
  async getPendingTransfers(
    ...
    try {
      ...
      // 2. Parse XML
      // 3. Prepare filters BEFORE parsing all nodes to save memory
      ...
      for (let i = 0; i < transferNodes.length; i++) {
        ...
        // Fast-fail 1: existing ID?
        const isAlreadyImported = existingTransferIds.has(String(transfer.id));
        if (isAlreadyImported) continue;

        // Fast-fail 2: monitored reservoir?
        const destId = transfer['pointing-in']?.['reservoir-id'];
        const isMonitored = destId && monitoredReservoirIds.has(String(destId));
        if (!isMonitored) continue;

        // Passed all checks, fetch operator name and save
        if (transfer['operator-id']) {
          transfer.operatorName = await this.getOperatorName(transfer['operator-id'], headers, config);
        }
        transfer.status = 'PENDENTE_DADOS';
