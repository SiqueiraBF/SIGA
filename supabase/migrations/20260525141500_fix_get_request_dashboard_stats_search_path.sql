-- Migration: Fix search_path for get_request_dashboard_stats
-- Description: The get_request_dashboard_stats RPC had its search_path set to '' (empty string), causing relation "solicitacoes" does not exist errors because the queries inside did not use fully qualified table names (e.g., public.solicitacoes). Changing search_path to public fixes this.
        ...
        'charts', json_build_object(
            ...
            'daily_volume', (
                ...
                FROM (
                    ...
                    SELECT
                        ds.d::DATE as date, -- Returned as YYYY-MM-DD
